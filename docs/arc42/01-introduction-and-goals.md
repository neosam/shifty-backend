...
| 3 | **Maintainability by a very small team** | The system must stay evolvable with minimal staffing. Strict layering (REST → Service → DAO), trait-based interfaces with mock-based unit tests, two-tier service rules preventing cyclic dependencies, spec-driven change workflow (OpenSpec). |

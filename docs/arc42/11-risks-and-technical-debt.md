...
| R4 | **Forgotten snapshot version bump** on formula change. *(§3)* | Old snapshots misdiagnosed as data bugs; audit value degraded. | OpenSpec conventions embed the bump rule; old-snapshot-read tests. No automated guard yet. |
...
| D3 | **Stale artifacts**: `docker.nix` references files that don't exist; `AUTHENTICATION.md` still documents the removed `MockContext`; `docs/ops/configuration.md` lists env names (`OIDC_ISSUER`, `PORT`) that differ from code (`ISSUER`, `SERVER_ADDRESS`); `docs/architecture/README.md` references `service-graph-traits.mmd` which is absent. | Delete or fix; cheap wins. |

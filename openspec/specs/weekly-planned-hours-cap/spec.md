# Weekly Planned Hours Cap

## Purpose

Provides an opt-in per-`EmployeeWorkDetails` cap that limits a week's shiftplan booking contribution to the expected hours for that week. When enabled, any overflow above expected hours is attributed to volunteer hours rather than being credited as overtime. The mechanism is one-sided (it does not pad weeks below expected) and is time-versioned alongside the rest of the `EmployeeWorkDetails` record.

...
use crate::{error::ShiftyError, loader, state::text_template::TextTemplate};
...
pub async fn load_text_templates_by_type(template_type: &str) -> Result<(), ShiftyError> {
    ...
    let templates =
        loader::load_text_templates_by_type(CONFIG.read().clone(), template_type).await?;
...
pub async fn update_text_template(
    ...
    let updated_template =
        loader::update_text_template(CONFIG.read().clone(), template_id, template).await?;
...
pub async fn delete_text_template(template_id: Uuid) -> Result<(), ShiftyError> {
    ...
    let templates: Vec<TextTemplate> = store
        .templates
        .iter()
    ...
    let filtered: Vec<TextTemplate> = store
        .filtered_templates
        .iter()
        .filter(|t| t.id != template_id)
        .cloned()
